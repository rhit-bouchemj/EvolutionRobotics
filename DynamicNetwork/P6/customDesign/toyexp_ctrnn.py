...
def activity(er):
    nn = ctrnn.CTRNN(size)
    nn.randomizeParameters()    #randomize weights, bias, and time constant

    # Set some correct proportion of excitatory/inhibitory connections
    for i in range(size):
        for j in range(size):
            if np.random.random() < er:
                nn.Weights[i,j] = np.random.random()*10.0 - 0.5 #Set value to -0.5 <-> 9.5 (excitatory connection)
            else:
                nn.Weights[i,j] = 0.0 #np.random.random()*(-10.0) #null connection (but could be inhibit)

    nn.initializeState(np.zeros(size))  #set init to 0s
    outputs = np.zeros((len(time),size))    #set init out to 0s
